...
export const AutomationCard = memo(function AutomationCard({
  ...
  // Memoize the parsed workflow config to avoid re-parsing on every render
  const workflowType = useMemo(() => {
    try {
      if (typeof automation.workflowConfig === 'string') {
        return JSON.parse(automation.workflowConfig).type;
      }
      return automation.workflowConfig.type;
    } catch (e) {
      console.error('Failed to parse workflow config', e);
      return 'Unknown';
    }
  }, [automation.workflowConfig]);
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="w-20 bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min((automation.riskScore / automation.maxRiskScore) * 100, 100)}%`,
                  }}
                />
              </div>
            </div>
